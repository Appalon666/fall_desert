...
// --- п.1.14: сцена осталась стоять без причины → сторож её поднимает ---
//
// Phaser разбирает очередь pause/resume только в игровом цикле, а на скрытой
// вкладке цикла нет: пара «пауза/снятие» может разъехаться и оставить сцену
// стоять, когда причин паузы уже не осталось. Игрок видит живую картинку, на
// которой ничего не нажимается, и лечит это только перезагрузкой страницы.
// Подделать это событиями нельзя — воспроизводим напрямую: останавливаем сцену
// мимо Pause и смотрим, поднимет ли её сторож.
await page.evaluate(() => {
  for (const sc of window.__yp.game.scene.getScenes(true)) sc.scene.pause()
})
// Промежуточное состояние НЕ проверяем: сторож ходит раз в секунду и успевает
// вклиниться между остановкой и замером — проверка выходила гоночной. Значение
// имеет только исход: сцена обязана подняться сама. Что проверка осмысленна,
// подтверждено откатом — без Pause.enforce она падает.
await page.waitForTimeout(2500)
const healed = await snapshot()
check('п.1.14 сторож поднял сцену без перезагрузки страницы',
  healed.running.length > 0, `идут: ${healed.running.join(', ') || 'ни одной'}`)
