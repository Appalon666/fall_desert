...
//   REVIEW — открыто окно оценки игры от платформы (feedback.requestReview).
...
//
// Причины ORIENTATION здесь больше нет: экран «поверни устройство» убран по
// п.1.15 (свои заглушки о повороте запрещены), а заодно исчез самый живучий
// источник вечной паузы — см. комментарий в main.js.
...
class PauseManager {
  ...
  // Пауза могла прийти ДО того, как Phaser поднял звук и сцены (вкладку
  // свернули на загрузке). Вызывается по 'ready', чтобы догнать состояние.
