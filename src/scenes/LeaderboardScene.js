...
export default class LeaderboardScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(GAME.WIDTH / 2, 96, `Твой рекорд: ${fmt(State.bestScore)} убийств`, { fontFamily: 'Rubik, sans-serif', fontSize: '20px', color: CSS.cap, fontStyle: 'bold' }).setOrigin(0.5)

    createButton(this, GAME.WIDTH / 2, GAME.HEIGHT - 44, { label: '⟵ В лагерь', width: 300, height: 50, onClick: () => this.scene.start(SCENES.HUB) })

    this.status = this.add.text(GAME.WIDTH / 2, 150, 'Загрузка таблицы…', { fontFamily: 'Rubik, sans-serif', fontSize: '18px', color: '#ddd2b4' }).setOrigin(0.5)
    this.loadBoard()
  }
  ...
  async loadBoard() {
    ...
    res.entries.slice(0, 10).forEach((e) => {
      ...
      this.add.text(cx + 300, y + 23, `${fmt(e.score)} убийств`, { fontFamily: 'Rubik, sans-serif', fontSize: '18px', color: CSS.toxic, fontStyle: 'bold' }).setOrigin(1, 0.5)
      y += 52
    })
  }
}
