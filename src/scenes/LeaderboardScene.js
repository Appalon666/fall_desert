...
export default class LeaderboardScene extends Phaser.Scene {
  ...
  create() {
    ...
    titleText(this, GAME.WIDTH / 2, 50, '🏆 РЕКОРДЫ', { size: 36 })
    this.add.text(GAME.WIDTH / 2, 96, `Твой рекорд: ${fmt(State.bestScore)} убийств`, { fontFamily: 'Trebuchet MS, sans-serif', fontSize: '20px', color: CSS.cap, fontStyle: 'bold' }).setOrigin(0.5)
  ...
  async loadBoard() {
    ...
    if (!res || !res.entries || !res.entries.length) {
      this.status.setText('Онлайн-таблица доступна в версии на Яндекс.Играх.\nЗдесь — твой личный рекорд.').setAlign('center')
      return
    }
    this.status.destroy()
    ...
    res.entries.slice(0, 10).forEach((e) => {
      ...
      this.add.text(cx - 300, y + 23, `#${e.rank}`, { fontFamily: 'Trebuchet MS, sans-serif', fontSize: '18px', color: CSS.cap, fontStyle: 'bold' }).setOrigin(0, 0.5)
      const name = (e.player && e.player.publicName) || 'Аноним'
      this.add.text(cx - 240, y + 23, name, { fontFamily: 'Trebuchet MS, sans-serif', fontSize: '18px', color: CSS.paper }).setOrigin(0, 0.5)
      this.add.text(cx + 300, y + 23, `${fmt(e.score)} убийств`, { fontFamily: 'Trebuchet MS, sans-serif', fontSize: '18px', color: CSS.toxic, fontStyle: 'bold' }).setOrigin(1, 0.5)
      y += 52
    })
  }
}
