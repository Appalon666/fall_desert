...
export default class PrestigeScene extends Phaser.Scene {
  ...
  render() {
    ...
    this.uiObjs = []
    this.coresText.setText(`${fmt(State.cores)} ядер`)
  ...
  confirmPrestige(gain) {
    ...
    const no = createButton(this, cx + 110, cy + 20, { label: 'Отмена', width: 200, height: 54, onClick: () => { ov.destroy(); t.destroy(); yes.destroy(); no.destroy() } })
