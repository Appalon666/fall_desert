...
export default class ForgeScene extends Phaser.Scene {
  ...
  render() {
    ...
    this.uiObjs = []
    this.scrapText.setText(`${fmt(State.scrap)} металлолома`)
    ...
    CRAFT_TIERS.forEach((t, i) => this.makeTier(startX + i * (cardW + gap), 210, cardW, t))
  }

  makeTier(cx, cy, w, t) {
    const h = 150
    const afford = State.scrap >= t.cost
    const c = this.add.container(cx, cy)
    const bg = this.add.graphics()
    const border = parseInt(t.css.slice(1), 16) // цвет рамки из t.css
    const paint2 = (hover) => {
      bg.clear()
    ...
    const btn = createButton(this, 0, h / 2 - 26, {
      label: afford ? 'Ковать' : 'Мало 🔩', width: w - 50, height: 40, fontSize: 18,
      color: afford ? COLORS.rust : COLORS.steelDark, hover: COLORS.rustLight, enabled: afford,
      onClick: () => this.doCraft(t.id),
    })
    c.add(btn)
    ...
    z.on('pointerup', () => { if (afford) this.doCraft(t.id) })
    c.add(z)
    this.uiObjs.push(c)
  }
