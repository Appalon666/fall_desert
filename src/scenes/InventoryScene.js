...
export default class InventoryScene extends Phaser.Scene {
  ...
  safeRender() {
    try { this.render() } catch (e) {
      ...
      this.add.text(GAME.WIDTH / 2, GAME.HEIGHT / 2, 'Не удалось открыть инвентарь.\nВернись в лагерь.', {
        fontFamily: 'Trebuchet MS, sans-serif', fontSize: '22px', color: '#ff6a6a', align: 'center',
      }).setOrigin(0.5)
    }
  }
  ...
  render() {
    ...
    const clsLabel = this.add.text(hx, hy + 150, cls ? `${cls.icon} ${cls.name}` : '', { fontFamily: 'Trebuchet MS, sans-serif', fontSize: '20px', color: CSS.cap, fontStyle: 'bold' }).setOrigin(0.5)
