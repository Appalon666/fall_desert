...
export const RARITIES = [
  ...
  { id: 'epic',     name: 'Эпическое', color: 0xb96ff0, css: '#b96ff0', weight: 3.4, mul: 5.0 },
  // Оранжевый легендарный — ПОТОЛОК ДОБЫЧИ. Раньше его не было вовсе: тир
  // фиолетового цвета назывался «Легенда», и лестница обрывалась на пятом шаге.
  // Теперь их шесть: серый → зелёный → синий → фиолетовый → оранжевый → красный.
  { id: 'legendary', name: 'Легендарное', color: 0xff9a1f, css: '#ff9a1f', weight: 0.6, mul: 8.0 },
  // mul реликвии = РОВНО ВДВОЕ от легендарного (8.0 → 16.0). Так высший тир и
  // ощущается высшим, а не «ещё чуть-чуть лучше». Разрыв стережёт тест
  // «реликвия ровно вдвое сильнее легендарного».
  ...
  // weight: 0 — реликвия НЕ ВЫПАДАЕТ. Ни с врагов, ни с боссов, ни с верстака:
  // единственный путь к ней — собрать пять частей и выковать. Иначе пропадает
  // смысл и в выбивании частей, и в прохождении локаций ради них.
  //
  // Была 0.6, и это ломало ровно то, ради чего реликвия делалась: удача даёт
  // множитель к весам с индекса 2, поэтому на 121 очке удачи (lootLuck = 6.05)
  // вес превращался в 0.6 × 7.05 из 202.85 — каждый сорок восьмой предмет
  // выпадал красным. Ковка обесценивалась полностью.
  //
  // Ковка выдаёт реликвию через forceRarityId ('relic'), а он таблицу весов
  // обходит, — поэтому нулевой вес её не ломает. Стережёт тест
  // «реликвия не выпадает из добычи».
  { id: 'relic',    name: 'Реликвия', color: 0xff3b30, css: '#ff3b30', weight: 0, mul: 16.0 },
]
...
function pickRarity(rng, luckBonus = 0) {
  // Нулевой вес — тир вне таблицы совсем (реликвия): фильтруем ДО броска, а не
  // полагаемся на то, что ноль сам себя не выберет. Без фильтра последний тир
  // с нулевым весом всё равно доставался бы броску, попавшему точно в хвост.
  const table = RARITIES
    .map((r, i) => ({ r, w: r.weight * (i >= 2 ? 1 + luckBonus : 1) }))
    .filter(t => t.w > 0)
...
const SCRAP_BASE = { common: 1, uncommon: 3, rare: 8, epic: 22, legendary: 40, relic: 60 }
...
export const CRAFT_TIERS = [
  ...
  // Ковка ЧАСТИ реликвии — не самой реликвии. Это принципиально: покупать за
  // лом готовую реликвию значило бы обесценить и выбивание частей с босса
  // 10-й локации, и прохождение локаций вообще. А часть — это ускорение уже
  // начатого пути: набор всё равно надо собрать из пяти и выковать вручную.
  //
  // Гарантированно НЕДОСТАЮЩАЯ часть, без лотереи: за такую цену дубликат
  // игрок бы не простил. Когда набор собран, тир гаснет — покупать нечего.
  { id: 'relicPart', name: 'Часть реликвии', cost: 10000, luck: 0, relicPart: true, css: '#ff3b30' },
]
