// Десять зон с разным визуалом и своим пулом врагов.
...
export function getZone(index) {
  ...
  // строку «Логово Босса · Шустрая» в словаре уже не найти.
  //
  // Номер петли («+36») из имени убран: рядом с ним всегда стоит номер локации
  // («ЗОНА 46 · ЛОГОВО БОССА +36»), и два числа об одном и том же читались как
  // ошибка. Само поле loop остаётся — по нему считается аффикс.
  const suffix = affix.name ? ` · ${t(affix.name)}` : ''
