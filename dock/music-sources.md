...
указание авторства НЕ требуется. Лицензия и источник продублированы в тегах
самих файлов (`LICENSE`, `SOURCE`) — модерации видно прямо из файла, без
переписки. У `menu.ogg` проставлены ещё `TITLE` и `ARTIST`, у `battle.ogg` их
нет. Дописывать не стали: файлы в нынешнем виде уже прошли проверку, а ради
двух необязательных при CC0 тегов пересобирать звук и архив — лишний риск.
